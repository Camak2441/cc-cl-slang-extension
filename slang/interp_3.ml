...
let map f = function
 ...
 | DEREF a  -> DEREF (f a)
 | ASSIGN a -> ASSIGN (f a)
 | MK_CLOSURE(a, loc, b)  -> MK_CLOSURE(f a, loc, b) 
...
let rec lookup (env, x) = 
    ...
    | (y, v) :: rest -> if x = y then Some(match v with
        | REC_CLOSURE(loc) -> CLOSURE(loc, (y, REC_CLOSURE loc)::rest, false)
        | _ -> v)
      else lookup (rest, x)
...
let rec string_of_value = function 
     ...
     | UNIT           -> "UNIT"
     | EMPTY          -> "EMPTY"
     | LIST(v1, v2)   -> "LIST(" ^ (string_of_value v1) ^ ", " ^ (string_of_value v2) ^ ")"
     | PAIR(v1, v2)    -> "(" ^ (string_of_value v1) ^ ", " ^ (string_of_value v2) ^ ")"
     | INL v           -> "inl(" ^ (string_of_value v) ^ ")"
     | INR  v          -> "inr(" ^ (string_of_value v) ^ ")"
     | CLOSURE (loc, c, b) -> "CLOSURE(" ^ (string_of_closure (loc, c)) ^ string_of_bool b^ ")"
...
and string_of_instruction = function
 ...
 | DEREF _  -> "DEREF"
 | ASSIGN _ -> "ASSIGN"
 | MK_CLOSURE(_, loc, b)  -> "MK_CLOSURE(" ^ (string_of_location loc) ^ ", " ^ string_of_bool b ^ ")"
...
let step (cp, evs) =
 ...
 | (MK_REF _,                   (V v) :: evs) -> let a = new_address () in (heap.(a) <- v;
                                               (cp + 1, V(REF a) :: evs))
 | (MK_CLOSURE (_, loc, b),                    evs) -> (cp + 1, V(CLOSURE(loc, evs_to_env evs, b)) :: evs)
 | (MK_REC (_, f, loc),                   evs) -> (cp + 1, V(CLOSURE(loc, (f, REC_CLOSURE loc):: evs_to_env evs, false)) :: evs)
 | (APPLY _,  (V v) :: V(CLOSURE ((_, Some i), env, false)) :: evs)
                                            -> (i, (V v) :: (EV env) :: (RA (cp + 1)) :: evs)
 | (STRICT a, V(CLOSURE ((_, Some i), env, true)) :: evs)
                                            -> (i, (EV env) :: (RA (cp + 1)) :: evs)
(* new intructions *)
...
type 'a defs_ref = {label: label ref; code: 'a instruction list ref}
type 'a shared_code = { const: 'a defs_ref }

let rec comp shared = function
  | Unit l           -> ([], [PUSH (l, UNIT)])
  | Empty l          -> ([], [PUSH (l, EMPTY)])
  | Integer(l, n)      -> ([], [PUSH (l, (INT n))])
  | Boolean(l, b)      -> ([], [PUSH (l, (BOOL b))])
  | Var(l, x)          -> ([], [LOOKUP (l, x)])
  | UnaryOp(l, op, e) -> let (defs, c) = comp shared e in  (defs, c @ [UNARY (l, op)])
  | Op(l, e1, op, e2) -> let (defs1, c1) = comp shared e1 in
                      let (defs2, c2) = comp shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [OPER(l, op)])
  | Pair(l, e1, e2)   -> let (defs1, c1) = comp shared e1 in
                      let (defs2, c2) = comp shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [MK_PAIR l])
  | Fst(l, e)          -> let (defs, c) = comp shared e in (defs, c @ [FST l])
  | Snd(l, e)          -> let (defs, c) = comp shared e in (defs, c @ [SND l])
  | Inl(l, e)          -> let (defs, c) = comp shared e in (defs, c @ [MK_INL l])
  | Inr(l, e)          -> let (defs, c) = comp shared e in (defs, c @ [MK_INR l])
  | Case(l, e1, (l', x1, e2), (l'', x2, e3)) ->
                      let inr_label = new_label () in
                      let after_inr_label = new_label () in
                      let (defs1, c1) = comp shared e1 in
                      let (defs2, c2) = comp shared e2 in
                      let (defs3, c3) = comp shared e3 in
                         (defs1 @ defs2 @ defs3,
                          (c1
 ...
 | Seq(_, [])         -> ([], [])
 | Seq(_, [e])        -> comp shared e
 | Seq(l, (e ::rest)) -> let (defs1, c1) = comp shared e in
                     let (defs2, c2) = comp shared (Seq (l, rest)) in
                       (defs1 @ defs2, c1 @ [POP l] @ c2)
 | Ref(l, e)          -> let (defs, c) = comp shared e in (defs, c @ [MK_REF l])
 | Deref(l, e)        -> let (defs, c) = comp shared e in (defs, c @ [DEREF l])
 ...
 | Assign(l, e1, e2) -> let (defs1, c1) = comp shared e1 in
                     let (defs2, c2) = comp shared e2 in
                         (defs1 @ defs2, c1 @ c2 @ [ASSIGN l])
 | App(l, e1, e2)    -> let (defs1, c1) = comp shared e1 in
                      let (defs2, c2) = comp shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [APPLY l])
 | Lambda(l, x, e)    -> let (defs, c) = comp shared e in
                      let f = new_label () in
                      let def = [LABEL (l, f) ; BIND(l, x)] @ c @ [SWAP l; POP l; RETURN l] in
                          (def @ defs, [MK_CLOSURE(l, (f, None), false)])
(*
...
*)
 | LetFun(l, f, (l', x, e1), e2) ->
                      ...
                      let def = [LABEL(l, lab); BIND(l',x)] @ c1 @ [SWAP l; POP l; RETURN l] in
                          (def @ defs1 @ defs2,
                           [MK_CLOSURE(l, (lab, None), false); BIND (l, f)] @ c2 @ [SWAP l; POP l])
 ...
 | Lazy(l, e) -> 
                      let (defs1, c) = comp shared e in
                      let lab = new_label () in
                      let def = LABEL(l, lab) :: c @ [RETURN l] in
                          (def @ defs1, [MK_CLOSURE(l, (lab, None), true)])
 | Strict(l, e) -> let (defs, c) = comp shared e in (defs, c @ [STRICT l])
 | StrictThunk(l, e) -> 
                      let (defs, c) = comp shared e in
                      if !(shared.const.code) = [] then shared.const.code := [LABEL(l, !(shared.const.label)); LOOKUP (l, "a"); SWAP l; POP l; RETURN l]
                      else ();
                      (defs, c @ [BIND(l, "a"); MK_CLOSURE(l, (!(shared.const.label), None), true); SWAP l; POP l])

let compile e =
    let shared = {const = {label = ref (new_label()); code = ref []}} in (* shared is used to share reused functions between instructions *)
    let (defs, c) = comp shared e in
...
let rec find lab = function 
   ...
   in let apply_label_map_to_instruction m = function 
     ...
     | MK_CLOSURE (l, (lab, _), b) -> MK_CLOSURE(l, (lab, Some(find lab m)), b)
