...
let do_strict s = function
  | THUNK f -> f s
  | _ -> complain "runtime error. Stricted non-thunk"
...
let rec interpret (e, env, store) =
    ...
    | Deref(_, e)          -> let (v, store') = interpret(e, env, store) in 
                            do_deref(v, store')
    | Assign(_, e1, e2)   -> let (v, store') = interpret(e1, env, store) in
                          (match v with
                          | REF a -> do_assign a (interpret(e2, env, store'))
                          | _ -> complain "runtime error : expecting an address on left side of assignment"
                          )
    | UnaryOp(_, op, e)   -> let (v, store') = interpret(e, env, store) in 
                          (do_unary(op, v), store')
    ...
    | LetRecFun(_, f, (_, x, body), e) ->
       ...
       in interpret(e, new_env, store)
    | Lazy(_, e) -> (THUNK (fun s -> interpret(e, env, s)), store)
    | Strict(_, e) -> let (v, store') =  interpret(e, env, store) in do_strict store' v
    | StrictThunk(_, e) -> let (v, store') =  interpret(e, env, store) in (THUNK (fun s -> (v, s)), store')
