...
type heap_item = 
  ...
  | HEAP_CI of code_index * bool    (* Code pointer for closures    *) 
...
(*  | BIND of var            not needed *)
  ...
  | MK_INR of 'a
  | MK_REF of 'a
  | MK_CLOSURE of 'a * location * int * bool (* modified *)
  | TEST of 'a * location
  | CASE of 'a * location
  | GOTO of 'a * location
  | LABEL of 'a * label
...
let map f = function
  ...
  | DEREF a  -> DEREF (f a)  
  | APPLY a  -> APPLY (f a)  
  | RETURN (a, n)  -> RETURN (f a, n)  
  | MK_PAIR a  -> MK_PAIR (f a)  
  | MK_INL a  -> MK_INL (f a)  
  | MK_INR a  -> MK_INR (f a)  
  | MK_REF a  -> MK_REF (f a)  
  | MK_CLOSURE(a, location, int, b) -> MK_CLOSURE(f a, location, int, b) 
...
let string_of_heap_item = function 
  ...
  | HEAP_CI (i, b)  -> "HEAP_CI " ^ (string_of_int i) ^ ", " ^ (string_of_bool b)
...
let string_of_instruction = function
 ...
 | APPLY _    -> "APPLY"
 | RETURN (_, n)   -> "RETURN" ^ (string_of_int n)
 | HALT _     -> "HALT"
 | LABEL(_, l)  -> "LABEL " ^ l
 | SWAP _     -> "SWAP"
 | POP _      -> "POP"
 | DEREF _    -> "DEREF"
 | ASSIGN _   -> "ASSIGN"
 | MK_CLOSURE (_, loc, n, b)
             -> "MK_CLOSURE(" ^ (string_of_location loc) 
	                      ^ ", " ^ (string_of_int n) ^ ", " ^ (string_of_bool b) ^ ")"
 | STRICT _ -> "STRICT"
...
let rec string_of_heap_value a vm = 
  ...
  | HEAP_HEADER (_, ht) -> 
    (match ht with 
    | HT_LIST -> "(" ^ (string_of_heap_value (a + 1) vm) ^ ", " ^ (string_of_heap_value (a + 2) vm) ^ ")"
...
let stack_to_heap_item = function 
  ...
  | STACK_EMPTY -> HEAP_EMPTY
  | STACK_HI i -> HEAP_HI i 
  | STACK_RA i -> HEAP_CI (i, false)
...
let heap_to_stack_item = function 
  ...
  | HEAP_EMPTY -> STACK_EMPTY
  | HEAP_HI i -> STACK_HI i 
  | HEAP_CI (i, _) -> STACK_RA i 
...
let mk_list vm = 
  ...
  let (v_left, vm2) = pop_top vm1 in 
  let (a, vm3) = allocate(3, vm2) in 
  let header = HEAP_HEADER (3, HT_LIST) in 
  let _ = Array.set vm.heap a header in 
  let _ = Array.set vm.heap (a + 1) (stack_to_heap_item v_left) in 
  let _ = Array.set vm.heap (a + 2) (stack_to_heap_item v_right) in 
      push(STACK_HI a, vm3) 
...
let do_hd vm = 
  ...
  | STACK_HI a -> 
    (match vm1.heap.(a) with 
    | HEAP_HEADER(_, HT_LIST) -> 
          push(heap_to_stack_item (vm.heap.(a + 1)), vm1) 
    | _ -> Errors.complain "do_head : unexpectd heap item"     
    )
  | _ -> Errors.complain "do_head : expecting heap pointer on stack"
...
let do_tl vm = 
    ...
    | STACK_HI a -> 
      ...
      | HEAP_HEADER(_, HT_LIST) -> 
            push(heap_to_stack_item (vm.heap.(a + 2)), vm1)      
      | _ -> Errors.complain "do_tl : unexpectd heap item"
      )
    | _ -> Errors.complain "do_tl : expecting heap pointer on stack"
...
let perform_unary(op, vm) = 
    ...
    | HEAD -> do_hd vm
    | TAIL -> do_tl vm
    | _ -> let (v, vm1) = pop_top vm in push(do_unary(op, v), vm1) 
...
let return (n, vm) = 
    ...
    | (STACK_FP saved_fp, STACK_RA k) -> 
       ...
       in push(return_value, { vm with cp = k; fp = saved_fp ; sp = current_fp - 1 - n}) 
...
let mk_closure = function 
  | ((_, Some i), n, b, vm) -> 
    ...
    let code_address = HEAP_CI (i, b)    in 
...
let strict vm = 
    ...
    | STACK_HI a -> 
        ...
        | HEAP_CI (i, true) -> 
          ...
          in let new_vm = {vm with cp = i; fp = new_fp }
          in push(return_index, push(saved_fp, new_vm))
        |_ -> Errors.complain "strict: runtime error, expecting code index in heap")
    | _ -> Errors.complain "strict: runtime error, expecting code index in heap"
...
let step vm =
 match get_instruction vm with
  ...
  | RETURN (_, n)            -> return (n, vm)
  | MK_CLOSURE(_, l, n, b)  -> advance_cp (mk_closure(l, n, b, vm))
...
let map_instruction_labels f = function 
     ...
     | MK_CLOSURE (l, ((lab, _)), n, b) -> MK_CLOSURE((l, (lab, Some(f lab)), n, b))
     | inst -> inst 
...
type 'a func_ref = {label: label ref; code: 'a instruction list ref}
type 'a shared = {const: 'a func_ref}

let rec comp vmap shared = function
  | Unit l           -> ([], [PUSH(l, STACK_UNIT)])
  | Empty l          -> ([], [PUSH(l, STACK_EMPTY)])
  | Boolean(l, b)      -> ([], [PUSH (l, STACK_BOOL b)])
  | Integer(l, n)      -> ([], [PUSH (l, STACK_INT n)])
  | UnaryOp(l, op, e) -> let (defs, c) = comp vmap shared e in  (defs, c @ [UNARY(l, op)])
  | Op(l, e1, op, e2) -> let (defs1, c1) = comp vmap shared e1 in
                      let (defs2, c2) = comp vmap shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [OPER(l, op)])
  | Pair(l, e1, e2)   -> let (defs1, c1) = comp vmap shared e1 in
                      let (defs2, c2) = comp vmap shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [MK_PAIR l])
  | Fst(l, e)          -> let (defs, c) = comp vmap shared e in (defs, c @ [FST l])
  | Snd(l, e)          -> let (defs, c) = comp vmap shared e in (defs, c @ [SND l])
  | Inl(l, e)          -> let (defs, c) = comp vmap shared e in (defs, c @ [MK_INL l])
  | Inr(l, e)          -> let (defs, c) = comp vmap shared e in (defs, c @ [MK_INR l])
  | Case(l, e1, (l', x1, e2), (l'', x2, e3)) ->
                      let inr_label = new_label () in
                      let after_inr_label = new_label () in
                      let (defs1, c1) = comp vmap shared e1 in
                      let (defs2, c2) = comp vmap shared (Lambda(l', x1, e2)) in
                      let (defs3, c3) = comp vmap shared (Lambda(l'', x2, e3)) in
                         (defs1 @ defs2 @ defs3,
                          (c1
  ...
  | If(l, e1, e2, e3) -> let else_label = new_label () in
                      ...
                      let (defs1, c1) = comp vmap shared e1 in
                      let (defs2, c2) = comp vmap shared e2 in
                      let (defs3, c3) = comp vmap shared e3 in
                         (defs1 @ defs2 @ defs3,
                          (c1
 ...
 | Seq (_, [])         -> ([], [])
 | Seq (_, [e])        -> comp vmap shared e
 | Seq (l, (e ::rest)) -> let (defs1, c1) = comp vmap shared e in
                     let (defs2, c2) = comp vmap shared (Seq(l, rest)) in
                       (defs1 @ defs2, c1 @ [POP l] @ c2)
 | Ref (l, e)          -> let (defs, c) = comp vmap shared e in (defs, c @ [MK_REF l])
 | Deref (l, e)        -> let (defs, c) = comp vmap shared e in (defs, c @ [DEREF l])
 ...
 | Assign(l, e1, e2) -> let (defs1, c1) = comp vmap shared e1 in
                     let (defs2, c2) = comp vmap shared e2 in
                         (defs1 @ defs2, c1 @ c2 @ [ASSIGN l])

 | App(l, e1, e2)    -> let (defs1, c1) = comp vmap shared e1 in
                     let (defs2, c2) = comp vmap shared e2 in
                          (defs1 @ defs2, c1 @ c2 @ [SWAP l; APPLY l])
 | Var(l, x)           -> ([], [LOOKUP(l, find vmap x)])
 | LetFun(l, f, (l', x, e1), e2) -> comp vmap shared (App(l, Lambda(l, f, e2), Lambda(l', x, e1)))
 | Lambda(l, x, e)           -> comp_lambda vmap shared (l, None, Some x, e, false)
 | LetRecFun(l, f, (l', x, e1), e2) ->
                      let (defs1, c1) = comp vmap shared (Lambda(l, f, e2)) in
                      let (defs2, c2) = comp_lambda vmap shared (l', Some f, Some x, e1, false) in
                          (defs1 @ defs2, c2 @ c1 @ [APPLY l])
 | Lazy(l, e) -> comp_lambda vmap shared (l, None, None, e, true)
 | Strict(l, e) -> let (defs, c) = comp vmap shared e in (defs, c @ [STRICT l])
 | StrictThunk(l, e) -> if !(shared.const.code) = [] then
                      shared.const.code := [LABEL(l, !(shared.const.label)); LOOKUP (l, HEAP_LOCATION (1)); RETURN (l, 0)]
                    else ();
                    let (defs, c) = comp vmap shared e in
                    (defs, c @ [MK_CLOSURE(l, (!(shared.const.label), None), 1, true)])

and comp_lambda vmap shared (l, f_opt, x_opt, e, b) =
    let bound_vars = match (f_opt, x_opt) with 
    | (None, None) -> []          
    | (None, Some x) -> [x] 
    | (Some f, None) -> [f]          
    | (Some f, Some x) -> [x; f] in
    let f = new_label () in
    let new_bind = match (f_opt, x_opt) with
    | (None, None) -> []
    | (None, Some x) -> [(x, STACK_LOCATION (-2))]
    | (Some f, None) -> [(f, STACK_LOCATION (-1))]
    | (Some f, Some x) -> [(x, STACK_LOCATION (-2)); (f, STACK_LOCATION (-1))] in
    ...
    let new_vmap = (new_bind @ env_bind) in
    let (defs, c) = comp new_vmap shared e in
    let def = [LABEL(l, f)] @ c @ [RETURN (l, if b then 0 else 1)] in
     (def @ defs, (List.rev fetch_fvars) @ [MK_CLOSURE(l, (f, None), List.length fvars, b)])

let compile e = 
    let shared = {const = {label = ref (new_label()); code = ref []}} in
    let (defs, c) = comp [] shared e in 
    let defs = !(shared.const.code) @ defs in
