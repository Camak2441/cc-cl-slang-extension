...
type oper = ADD | MUL | DIV | SUB | LT | AND | OR | EQB | EQI | CONS

type unary_oper = NEG | NOT | READ | HEAD | TAIL
...
| Lazy(a, e) -> Lazy (f a, map f e)
| Strict(a, e) -> Strict (f a, map f e)
| StrictThunk(a, e) -> StrictThunk (f a, map f e)
