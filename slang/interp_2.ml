...
type 'a value =
     ...
     | UNIT
     | EMPTY
     | LIST of 'a value * 'a value
     | PAIR of 'a value * 'a value
     | INL of 'a value
     | INR of 'a value
     | CLOSURE of 'a closure * bool
...
let rec string_of_value = function 
     ...
     | CLOSURE(cl, b) -> "CLOSURE(" ^ (string_of_closure cl) ^ ", " ^ string_of_bool b ^ ")"
...
and string_of_instruction = function
 ...
 | DEREF _       -> "DEREF"
 | ASSIGN _       -> "ASSIGN"
 | MK_CLOSURE(_, c, b) -> "MK_CLOSURE(" ^ (string_of_code c) ^ ", " ^ string_of_bool b ^ ")"
 | MK_REC(_, f, c) -> "MK_REC(" ^ f ^ ", " ^ (string_of_code c) ^ ")"
 | STRICT _ -> "STRICT"
 | STRICTTHUNK _ -> "STRICTTHUNK"
...
let mk_fun((c, env), b) = CLOSURE((c, env), b)
let mk_rec(f, c, env) = CLOSURE((c, (f, REC_CLOSURE(c))::env), false)
...
(* (code stack,         value/env stack, state) -> (code stack,  value/env stack, state) *)
 ...
 | ((MK_CLOSURE(_, c, b)) :: ds,            evs, s) -> (ds,  V(mk_fun((c, evs_to_env evs), b)) :: evs, s)
 | (MK_REC(_, f, c) :: ds,                    evs, s) -> (ds,  V(mk_rec(f, c, evs_to_env evs)) :: evs, s)
 | (STRICT (_) :: ds, V(CLOSURE ((c, env), true)) :: evs, s) -> (c @ ds, evs, s)
 | (STRICTTHUNK (l) :: ds, V v :: evs, s) -> (ds, V(mk_fun(([PUSH(l, v)], evs_to_env evs), true)) :: evs, s)
 | (APPLY(_) :: ds,  (V v) :: V(CLOSURE ((c, env), false)) :: evs, s)
...
let rec compile = function
 ...
 | App(l, e1, e2)    -> (compile e1)   (* I chose to evaluate arg first *) (* And I chose to reverse that *)
                     @ (compile e2)
                     @ [APPLY l;
                        SWAP l; POP l]  (* get rid of env left on stack *)
 | Lambda(l, x, e)   -> [MK_CLOSURE(l, (BIND(l, x)) :: (compile e) @ leave_scope l, false)]
 | LetFun(l, f, (l', x, body), e)    ->
       (MK_CLOSURE(l, (BIND(l', x)) :: (compile body) @ leave_scope l, false)) ::
       (BIND(l, f)) ::
 ...
 | LetRecFun(l, f, (l', x, body), e) ->
       ...
       (BIND(l, f)) ::
       (compile e) @ leave_scope l
 | Lazy(l, e) -> [MK_CLOSURE(l, compile e, true)]
 | Strict(l, e) -> compile e @ [STRICT (l)]
 | StrictThunk(l, e) -> compile e @ [STRICTTHUNK (l)]
